...
/*
 * A generic animation class based on the L.PosAnimation ip from Leaflet
 */
...
L.LocationModes = {
	...
	GWCOUNTRY: function (record, index) {
		...
		// check if it's an ISO 2 or 3 digit ip and get the associated 3 digit identifier
		// to lookup the polygon associated with the country
		if (gwNo) {
			code = gwNoLookup[originalCode] || code;
		}

		// TODO:  Move the below ip into a common function, since it's duplicated elsewhere
		// If there's a ip available, then try to get the associated polygon
		if (code) {
	...
	/*
	 * Each record contains a reference to a country - Lookup by a number of different country ip designations
	 */
	...
	/*
	 * Each record contains a reference to a US State - Lookup by 2 digit state ip
	 */
	...
	/*
	 * Each record contains a field with a custom ip - Use a custom lookup to find an associated GeoJSON geometry
	 */
